...
func parsePAssertedIdGetUri(p *PAssertedId) pAssertedIdStateFn {
    ...
    if left < right {
        ...
        if p.URI.Error != nil {
            p.Error = os.NewError("parseRpidGetUri err: received err getting uri: " + p.URI.Error.String())
            return nil
        }
        return parsePAssertedIdGetParams
    }
    p.Error = os.NewError("parseRpidGetUri err: could not locate bracks.  no uri found.")
    return nil
}
