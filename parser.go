...
func (s *SipMsg) addHdr(str string) {
    ...
    if sp == -1 {
	s.Error = os.NewError("addHdr err: no semi found in: " + str)
	return
    }
...
func (s *SipMsg) GetCallingParty(str string) os.Error {
    switch {
    ...
    return os.NewError("GetCallingParty err: unknown err.  Fell through switch stmnt.")
}

func (s *SipMsg) getCallingPartyDefault() os.Error {
    if s.From == nil {
	return os.NewError("getCallingPartyDefault err: no from header found.")
    }
    if s.From.URI == nil {
	return os.NewError("getCallingPartyDefault err: no uri found in from header.")
    }
    s.CallingParty = &CallingPartyInfo{Name: s.From.Name, Number: s.From.URI.User,}
    return nil
}

func (s *SipMsg) getCallingPartyPaid() os.Error {
	...
	if s.PAssertedId.URI == nil {
	    return os.NewError("getCallingPartyPaid err: p-asserted-id uri is nil.")
	}
...
func (s *SipMsg) getCallingPartyRpid() os.Error {
	...
	if s.RemotePartyId.URI == nil {
	    return os.NewError("getCallingPartyRpid err: remote party id uri is nil.")
	}
...
func (s *SipMsg) parseRecordRoute(str string) {
	...
	if left < right {
	    ...
	    if u.Error != nil {
		s.Error = os.NewError("parseRecordRoute err: received err parsing uri: " + u.Error.String())
		return
	    }
...
func (s *SipMsg) parseRoute(str string) {
	...
	if left < right {
	    u := ParseURI(cs[rt][left + 1:right])
	    if u.Error != nil {
		s.Error = os.NewError("parseRoute err: received err parsing uri: " + u.Error.String())
		return
	    }
	    if s.Route == nil {
		s.Route = []*URI{u,}
...
func (s *SipMsg) parseStartLine(str string) {
    ...
    if s.StartLine.Error != nil {
        s.Error = os.NewError("parseStartLine err: received err while parsing start line: " + s.StartLine.Error.String())
    }
}
...
func ParseMsg(str string) (s *SipMsg) {
    ...
    if s.eof == -1 {
	s.Error = os.NewError("ParseMsg: err parsing msg.  No SIP eof found.")
	return s
    }
    s.run()
    return s
