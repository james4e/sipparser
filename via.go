...
func parseViaGetProto(v *Via) viaStateFn {
    ...
    if v.protoEnd == -1 {
	v.Error = os.NewError("parseViaGetProto err: could not get LWS char.")
	return nil
    }
    protoParts := strings.SplitN(v.Via[0:v.protoEnd], "/", 3)
    if len(protoParts) != 3 {
	v.Error = os.NewError("parseViaGetProto err: split err on proto char.")
	return nil
    }
...
func parseViaGetHostPort(v *Via) viaStateFn {
    if v.protoEnd == 0 {
	v.Error = os.NewError("parseViaGetHostPort err: protoEnd is 0.")
	return nil
    }
