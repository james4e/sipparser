...
func (a *Authorization) parse() os.Error {
    pos := strings.IndexRune(a.Val, ' ')
    if pos == -1 {
	return os.NewError("Authorization.parse err: no LWS found.")
    }
    a.Credentials = a.Val[0:pos]
    if len(a.Val) - 1 <= pos {
	return os.NewError("Authorization.parse err: no digest-resp found.")
    }
